...
void bridge_init( )
{
    __android_log_print(ANDROID_LOG_INFO, "BRAWL", "bridge_init" );
...
void bridge_open( char* url )
{
    ...
    jclass clazz = (*javaEnv)->FindClass(javaEnv,"com/milgra/brawl/Brawl");
...
JNIEXPORT jstring JNICALL Java_com_milgra_brawl_Brawl_storeitem(
        JNIEnv* env,
        jobject cls,
        jint index,
        jstring name)
...
JNIEXPORT jstring JNICALL Java_com_milgra_brawl_Brawl_showdonate(
        JNIEnv* env,
        jobject cls,
        jint index,
        jstring name)
